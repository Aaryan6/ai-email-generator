...
export const saveChatMessages = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const oldMessages = await ctx.db
      ...
      .collect();
    const oldByMessageId = new Map(oldMessages.map((row) => [row.messageId, row]));
    const incomingMessageIds = new Set<string>();

    const inserted: Array<{ id: string; dbId: string; role: unknown; parts: unknown[] }> = [];
    for (const [index, input] of (args.messages as MessageInput[]).entries()) {
      ...
      inserted.push({ id: messageId, dbId: String(dbId), role: input.role, parts });
    }
