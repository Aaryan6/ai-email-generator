...
const extractStyleProfile = (htmlCode: string): StyleProfile => {
  const colorMatches = htmlCode.match(/#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6,8})\b|rgba?\([^)]*\)|hsla?\([^)]*\)/g) ?? [];
  ...
  const spacingMatches = htmlCode.match(/(?:padding|margin)\s*:\s*([^;"']+)/gi) ?? [];
  const buttonStyleBlocks = htmlCode.match(/<(?:a|button)[^>]*style="[^"]*"/gi) ?? [];

  const buttonBackgrounds = uniqueNormalized(
    buttonStyleBlocks
      .map((block) => block.match(/background(?:-color)?\s*:\s*([^;"']+)/i)?.[1] ?? "")
      .filter(Boolean),
  ).slice(0, 8);
